...
func getCompiledProgram(statedb vm.StateDB, moduleHash common.Hash, addressForLogging common.Address, code []byte, codehash common.Hash, maxWasmSize uint32, maxFragments uint16, pagelimit uint16, time uint64, debugMode bool, program Program, runCtx *core.MessageRunContext) (map[rawdb.WasmTarget][]byte, error) {
	...
	wasm, err := getWasmFromContractCode(statedb, code, maxWasmSize, maxFragments)
	if err != nil {
...
func cacheProgram(db vm.StateDB, module common.Hash, program Program, addressForLogging common.Address, code []byte, codehash common.Hash, params *StylusParams, debug bool, time uint64, runCtx *core.MessageRunContext) {
	if runCtx.IsCommitMode() {
		...
		asmMap, err := getCompiledProgram(db, module, addressForLogging, code, codehash, params.MaxWasmSize, params.MaxFragmentCount, params.PageLimit, time, debug, program, runCtx)
		var ok bool
		var localAsm []byte
		if asmMap != nil {
