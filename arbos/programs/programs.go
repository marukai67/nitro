...
func (p Programs) ActivateProgram(evm *vm.EVM, address common.Address, runCtx *core.MessageRunContext, debugMode bool) (
	...
	wasm, err := getWasm(statedb, address, params.MaxWasmSize, params.MaxFragmentCount)
	if err != nil {
		return 0, codeHash, common.Hash{}, nil, false, err
	}
...
func (p Programs) CallProgram(
	...
	asmMap, err := getCompiledProgram(statedb, moduleHash, contract.Address(), contract.Code, contract.CodeHash, params.MaxWasmSize, params.MaxFragmentCount, params.PageLimit, evm.Context.Time, debugMode, program, runCtx)
	var ok bool
	var localAsm []byte
	if asmMap != nil {
...
func getWasm(statedb vm.StateDB, program common.Address, maxWasmSize uint32, maxFragments uint16) ([]byte, error) {
	prefixedWasm := statedb.GetCode(program)
	return getWasmFromContractCode(statedb, prefixedWasm, maxWasmSize, maxFragments)
}

func getWasmFromContractCode(statedb vm.StateDB, prefixedWasm []byte, maxWasmSize uint32, maxFragments uint16) ([]byte, error) {
	if len(prefixedWasm) == 0 {
		return nil, ProgramNotWasmError()
	}

	if state.IsStylusProgramClassic(prefixedWasm) {
		return handleClassicStylus(prefixedWasm, maxWasmSize)
	}

	if state.IsStylusProgramRoot(prefixedWasm) {
		return handleRootStylus(statedb, prefixedWasm, maxWasmSize, maxFragments)
	}
...
func handleRootStylus(statedb vm.StateDB, data []byte, maxSize uint32, maxFragments uint16) ([]byte, error) {
	rawAddresses, err := state.StripStylusRootPrefix(data)
	if err != nil {
		return nil, err
	}
	...
	for i := 0; i < len(rawAddresses); i += common.AddressLength {
		...
		compressedWasm = append(compressedWasm, payload[1:]...)
	}
	...
	return arbcompress.DecompressWithDictionary(compressedWasm, int(maxSize)*2, dict)
}
